...
func (p *parser) setDefaultOptions() {
	...
	if p.blockchainQuerier == nil {
		p.blockchainQuerier = cloudflareeth.NewCloudflareEth()
	}
...
func (p *parser) initScannedBlock() error {
	...
	p.lastScannedBlock.Store(int64(blockNumber))
	return nil
}
...
func (p *parser) querySubscribedAddressTransactions(ctx context.Context) {
	...
	for i := p.lastScannedBlock.Load() + 1; i <= currentBlock; i++ {
		select {
		case <-ctx.Done():
			slog.Info(fmt.Sprintf("scanning block %d stopped", i))
			return
		default:
			p.saveSubscribedAddressTransactions(p.getTransactionsInBlock(i))
			p.lastScannedBlock.Store(i)
		}
	}
}
...
func (p *parser) saveSubscribedAddressTransactions(transactions []blockchain.Transaction) {
	for _, t := range transactions {
		if addr, ok := p.datastore.Get(t.From); ok {
			err := p.datastore.Add(t.From, append(addr, t))
			if err != nil {
				// if this happens we probably want to do some more than logging it.
				slog.Error(fmt.Sprintf("error storing transaction %s for address %s %v", t.String(), addr, err))
			}
		}
		if addr, ok := p.datastore.Get(t.To); ok {
			err := p.datastore.Add(t.To, append(addr, t))
			if err != nil {
				// if this happens we probably want to do some more than logging it.
				slog.Error(fmt.Sprintf("error storing transaction %s for address %s %v", t.String(), addr, err))
			}
...
func (p *parser) startBlockScanning(ctx context.Context) {
	if ctx == nil {
